...
import { type AVLData, type IOElement } from './types/type';
...
export default class PacketDecoder {
  ...
  private parseAndDecodeCodec8EData() {
    ...
    const extendedEventIoId = this._parser.readData(
      2,
      DataType.ExtendedEventIoId,
    );
    ...
    for (let j = 0; j < ioCountXBData.readInt16BE(0); j++) {
      ...
      const extendedElementLength = this._parser.readData(
        2,
        DataType.ExtendedElementLength,
      );
      const value = this._parser.readData(
        extendedElementLength.readInt16BE(0),
        DataType.ExtendedIoValueXB,
      );
